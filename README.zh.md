...
> 一个有主见的 AI agent **skill**，帮你搭起个人职业中枢——一个私人 Git 仓库，**既是 (a) 你的单一事实来源**（简历、面试准备、课题申报、投递前核查），**也是 (b) 你的职业罗盘**：往上 1–2 级你想坐的那把椅子是哪把、你和那把椅子之间明确的差距是什么、这个季度打算先补哪一块。任何能读 `AGENTS.md` 的 AI IDE 都能用（Claude Code、Cursor、Codex、Cline、Windsurf、GitHub Copilot……）。

生态里多数职业类 skill 都是一次性工具（"贴 JD 出简历"），这个不是。它是一套**框架**：先搭出一个持久、版本化的 hub，然后基于它支撑长期的求职、面试、课题申报和事实核查。**出简历只是一半价值——另一半是方向**：这个 hub 逼你先写下一个 stretch 目标，把能力差距（技能、title、scope、薪酬）摊开来看，每次投递前都回顾一次，让你的每次投递都在往一个明确的终点靠拢，而不是随机游走。

## 适合谁用

- 想把分散在各处的简历整合到一个单一事实来源里的人
- 想往上跳 **1–2 级**、但还没把差距和路径摊开来的人，或者还没想清楚下一步要瞄准哪把椅子的人
...
### 2. Agent 在 hub 上执行的六种工作流
- **职业规划 & 差距盘点** — 定一个向上 1-2 级的 stretch 目标，按四维框架（技能 / 范围 / 背书 / 人脉）摊差距，转成本季度的 SMART 计划。初始化后第一个跑，之后每季度再跑一次。
...
- **T 型 / π 型人才** — 职业定位
- **Career capital**（Cal Newport）— 职业资本与目标座位的差距盘点
...
或手动 clone 到你 IDE 的 skills 目录（下面给的是 Claude Code 的路径，Cursor / Codex / Cline / Windsurf 各自路径不同，自行调整）：
```bash
