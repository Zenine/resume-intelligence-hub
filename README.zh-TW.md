...
> 一個有主見的 AI agent **skill**，幫你搭起個人職涯中樞——一個私人 Git 儲存庫作為履歷、面試準備、研究計畫申報、投遞前查核的單一事實來源。任何能讀 `AGENTS.md` 的 AI IDE 都能用（Claude Code、Cursor、Codex、Cline、Windsurf、GitHub Copilot……）。

生態圈裡多數職涯類 skill 都是一次性工具（「貼 JD 出履歷」），這個不是。它是一套**框架**：先搭出一個持久、版本化的 hub，然後基於它支撐長期的求職、面試、研究計畫申報和事實查核。
...
透過 Skills CLI（推薦，等發布到 skills.sh 後）：
```bash
...
在 AI IDE 裡說任意一句：
- "帮我搭个简历库" / "build my career repo" — 從零開一個 hub
- "帮我针对这个 JD 生成简历" / "tailor my resume for this JD" — 客製履歷
- "看看有什么合适的机会" / "what's out there for me" — 主動搜職缺
- "针对这个职位帮我准备面试" / "prep me for this interview" — 面試輔導
- "投递前帮我核查一下公开资料" / "run a pre-submission verification" — 事實查核
- "帮我生成国自然基金简历" / "generate an NIH application profile" — 研究計畫申報（需開啟研究軌）
